...
gcloud container clusters create ${CLUSTER} --region ${REGION} --machine-type "${GKE_MACHINE_TYPE}" --network "${NETWORK}" --subnetwork "${SUBNETWORK}" --num-nodes "${GKE_NODES_PER_ZONE}" --min-nodes "${GKE_NODES}" --max-nodes "${GKE_NODES}" --addons HorizontalPodAutoscaling,HttpLoadBalancing,GcePersistentDiskCsiDriver --enable-ip-alias --scopes storage-rw --enable-shielded-nodes --enable-private-nodes --master-ipv4-cidr "172.14.0.0/28" --enable-master-authorized-networks --master-authorized-networks="0.0.0.0/0" --node-locations "$ZONE","$ZONE2" --enable-autoupgrade --enable-autorepair
