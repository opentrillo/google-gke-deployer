...
then
  ...
  gcloud beta sql instances create $MYSQL_INSTANCE_NAME --tier=${DB_MACHINE_TYPE} --activation-policy=ALWAYS --zone=$ZONE --no-assign-ip --database-version=MYSQL_5_7 --network=$NETWORK --backup --backup-start-time=03:00  --root-password=${ROOT_PASSWORD} --storage-auto-increase --database-flags=innodb_strict_mode=off
fi
